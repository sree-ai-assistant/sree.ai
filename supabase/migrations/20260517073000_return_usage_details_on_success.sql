-- ============================================================
-- Migration: Return Usage Details on Success
-- Description: Updates increment_multi_usage to populate and return the actual daily limits and counts even when requests are allowed.
-- ============================================================
...
DECLARE
  ...
  v_anon public.anonymous_users%ROWTYPE;
...
BEGIN
  ...
  -- 4. Commit Increments
  IF v_all_allowed AND p_increment THEN
  ...
  IF v_all_allowed THEN
    SELECT 
      ...
      CASE WHEN p_increment THEN COALESCE(u.daily_count, x.amount) ELSE COALESCE(u.daily_count, 0) END
    INTO v_limit, v_used
